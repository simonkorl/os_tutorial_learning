...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        ...
        match tuple{
        (r, g, b) if valid_range.contains(&r) && valid_range.contains(&g) && valid_range.contains(&b)
            => Ok(Color{red: r as u8, green: g as u8, blue: b as u8}),
        _ => Err(Self::Error::from("Tuple Try From Error"))
        }
    }
}
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        ...
        match arr{
            [r, g, b] if valid_range.contains(&r) && valid_range.contains(&g) && valid_range.contains(&b)
                => Ok(Color{red: r as u8, green: g as u8, blue: b as u8}),
            _ => Err(Self::Error::from("Array Try From Error"))
        } 
    }
}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {
            Err(Self::Error::from("Slice Try From Error"))
        } else {
            let valid_range = 0..=255;
            if valid_range.contains(&slice[0]) && valid_range.contains(&slice[1]) && valid_range.contains(&slice[2]) {
                Ok(Color {
                    red: slice[0] as u8,
                    green: slice[1] as u8,
                    blue: slice[2] as u8
                ...
                Err(Self::Error::from("Slice Try From Error in else"))
            }
        }
    }
}
